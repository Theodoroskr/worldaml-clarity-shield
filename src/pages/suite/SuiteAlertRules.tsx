...
/* EU AMLD (Anti-Money Laundering Directives) Regulatory Mapping */
const EU_AMLD_REQUIREMENTS: FinCENRequirement[] = [
  { id: "amld6-ml", regulation: "ML Predicate Offences (AMLD6)", citation: "Directive (EU) 2018/1673 Art 2", description: "AMLD6 harmonises and expands the list of 22 predicate offences for money laundering across all EU Member States, including tax crimes, cybercrime, and environmental crimes. Aiding, abetting, and attempting ML are also criminalised.", rulePatterns: ["HRCOU", "RISKWORD", "REFTEXT"], suggestedRules: [{ name: "[EU-AMLD6] Predicate offence keyword screening", severity: "high", rationale: "AMLD6 Art 2 expands predicate offences to 22 categories. Screening transactions for keywords linked to tax evasion, fraud, cybercrime, and environmental crime ensures coverage of the expanded scope.", conditions: [{ field: "transaction.type", operator: "CONTAINS", value: "predicate_keyword" }] }] },
  { id: "amld5-cdd", regulation: "Customer Due Diligence (AMLD5)", citation: "Directive (EU) 2018/843 Art 13", description: "Obliged entities must apply CDD when establishing a business relationship, carrying out occasional transactions ≥ €15,000 (or linked), or when there is suspicion of ML/TF. Simplified CDD is available for lower-risk situations; Enhanced CDD for high-risk third countries.", rulePatterns: ["VC", "CDC01-P", "CDC01-E", "MCOC", "OCMC"], suggestedRules: [{ name: "[EU-AMLD] Occasional transaction ≥ €15,000", severity: "high", rationale: "AMLD5 Art 11 requires CDD for occasional transactions of €15,000+. This rule flags transactions at the EU threshold.", conditions: [{ field: "transaction.amount", operator: ">", value: "15000" }] }, { name: "[EU-AMLD] Simplified CDD override for low-risk", severity: "low", rationale: "AMLD5 Art 15 permits simplified CDD for demonstrably lower-risk relationships. This rule identifies candidates for streamlined processing.", conditions: [{ field: "customer.riskScore", operator: "<", value: "3" }] }] },
  { id: "amld5-edd", regulation: "Enhanced Due Diligence — High-Risk Third Countries", citation: "Directive (EU) 2018/843 Art 18a", description: "Obliged entities must apply enhanced due diligence for business relationships or transactions involving high-risk third countries identified by the European Commission (Delegated Regulation). Measures include obtaining additional information on source of wealth/funds, senior management approval, and enhanced monitoring.", rulePatterns: ["HRCOU", "NCOU"], suggestedRules: [{ name: "[EU-AMLD] High-risk third country EDD trigger", severity: "critical", rationale: "AMLD5 Art 18a mandates EDD for all dealings with EU-listed high-risk third countries. This rule flags transactions involving jurisdictions on the Commission's delegated list.", conditions: [{ field: "transaction.country", operator: "IN", value: "AF,MM,SY,YE,KP,IR,PK" }, { field: "transaction.amount", operator: ">", value: "0" }] }] },
  { id: "amld5-pep", regulation: "PEP Requirements (EU)", citation: "Directive (EU) 2018/843 Art 20–23", description: "Obliged entities must have risk-based procedures to determine whether a customer or beneficial owner is a PEP, family member, or close associate. For PEPs: senior management approval, source of wealth/funds measures, and enhanced ongoing monitoring. Domestic PEPs and international organisation officials are included.", rulePatterns: ["CUSTPEP", "CTPYPEP"], suggestedRules: [{ name: "[EU-AMLD] PEP family & close associate screening", severity: "high", rationale: "AMLD5 Arts 20-23 extend PEP obligations to family members and known close associates. This rule flags related parties of identified PEPs.", conditions: [{ field: "customer.pepStatus", operator: "IN", value: "pep,family_pep,associate_pep" }] }] },
  { id: "amld5-ubo", regulation: "Beneficial Ownership Registers (UBO)", citation: "Directive (EU) 2018/843 Art 30–31", description: "Member States must ensure corporate entities hold adequate, accurate, and current information on their beneficial owners. Central registers must be accessible to FIUs, competent authorities, and (for companies) the public. A 25%+1 share threshold applies.", rulePatterns: ["MCOC", "OCMC"], suggestedRules: [{ name: "[EU-AMLD] UBO verification gap (>25% threshold)", severity: "high", rationale: "AMLD5 Art 30 requires identification of all beneficial owners holding >25% of shares. This rule flags entity customers with incomplete or outdated UBO records.", conditions: [{ field: "customer.status", operator: "==", value: "ubo_incomplete" }] }] },
  { id: "amld5-fiu", regulation: "FIU Suspicious Transaction Reporting", citation: "Directive (EU) 2018/843 Art 33", description: "Obliged entities must promptly report suspicious transactions to the national Financial Intelligence Unit (FIU). There is no minimum transaction threshold. Member States may extend reporting to tax-related suspicions. The FIU serves as the single national centre for receiving, analysing, and disseminating STRs.", rulePatterns: ["HRCOU", "DORMANT", "NCOU", "RISKWORD", "IN>AVG", "OUT>AVG"], suggestedRules: [{ name: "[EU-AMLD] FIU reporting — no threshold suspicion", severity: "critical", rationale: "AMLD5 Art 33 requires reporting to the FIU when there is knowledge or suspicion of ML/TF, regardless of amount. This rule flags any transaction with elevated risk indicators.", conditions: [{ field: "customer.riskScore", operator: ">", value: "7" }] }] },
  { id: "amld5-sanctions", regulation: "EU Sanctions & Restrictive Measures", citation: "Council Regulation (EC) No 881/2002 et seq.", description: "All EU persons and entities must comply with EU restrictive measures (sanctions). Assets of designated persons must be frozen. No funds may be made available directly or indirectly. Screening against EU Consolidated Sanctions List is mandatory.", rulePatterns: ["CUSTSCRS", "CUSTSCRH", "CTPYSCRS", "CUSTBIC", "CTPYBIC", "INSTSCRS", "INSTSCRH", "High-Risk Country"], suggestedRules: [{ name: "[EU-AMLD] EU Consolidated Sanctions List screening", severity: "critical", rationale: "EU Regulation requires screening against the EU Consolidated Financial Sanctions List (maintained by DG FISMA). This rule ensures all customers and counterparties are checked.", conditions: [{ field: "customer.status", operator: "==", value: "sanctions_match" }] }] },
  { id: "amld5-record", regulation: "Record Keeping (5 Years)", citation: "Directive (EU) 2018/843 Art 40", description: "Obliged entities must retain CDD documentation and transaction records for at least 5 years after the business relationship has ended or the occasional transaction completed. Records must permit reconstruction of transactions.", rulePatterns: [] },
  { id: "amld5-wire", regulation: "Wire Transfer Regulation (EU)", citation: "Regulation (EU) 2015/847 Art 4–7", description: "Payment service providers must ensure that transfers of funds are accompanied by information on the payer and payee. For transfers ≥ €1,000, full payer information (name, account, address/DOB/ID) must be included. Intermediary PSPs must retain all information.", rulePatterns: ["SUMCCI", "SUMCCO"], suggestedRules: [{ name: "[EU-WTR] Wire transfer missing payer info ≥ €1,000", severity: "high", rationale: "EU Wire Transfer Regulation Art 4 requires full payer information for transfers of €1,000+. This rule flags transfers with incomplete originator data.", conditions: [{ field: "transaction.amount", operator: ">", value: "1000" }, { field: "transaction.type", operator: "==", value: "wire_transfer" }] }] },
  { id: "amld5-virtual", regulation: "Virtual Asset Service Providers (AMLD5)", citation: "Directive (EU) 2018/843 Art 2(1)(g)", description: "AMLD5 brought virtual currency exchange platforms and custodian wallet providers into scope as obliged entities. They must register with national authorities and comply with full AML/CFT requirements including CDD, record-keeping, and suspicious transaction reporting.", rulePatterns: [], suggestedRules: [{ name: "[EU-AMLD] VASP transaction monitoring ≥ €1,000", severity: "high", rationale: "AMLD5 brings VASPs into scope. This rule monitors virtual currency transactions above €1,000 for suspicious patterns.", conditions: [{ field: "transaction.currency", operator: "IN", value: "BTC,ETH,USDT,USDC" }, { field: "transaction.amount", operator: ">", value: "1000" }] }] },
  { id: "amld5-risk-assess", regulation: "Supranational Risk Assessment", citation: "Directive (EU) 2018/843 Art 6", description: "The European Commission conducts a supranational ML/TF risk assessment at least every 2 years. Member States must take the Commission's findings into account in their national risk assessments. Obliged entities must consider both in their own risk assessments.", rulePatterns: [], suggestedRules: [{ name: "[EU-AMLD] Supranational risk factor alignment check", severity: "medium", rationale: "AMLD5 Art 6 requires alignment with the Commission's supranational risk assessment. This rule triggers a periodic review to ensure risk factors reflect current EU-level findings.", conditions: [{ field: "alert.count", operator: ">=", value: "0" }] }] },
];

/* CySEC (Cyprus Securities and Exchange Commission) Regulatory Mapping */
const CYSEC_REQUIREMENTS: FinCENRequirement[] = [
  { id: "cysec-aml", regulation: "AML/CFT Compliance (CySEC Directive)", citation: "CySEC Directive DI144-2007-08 / L.188(I)/2007", description: "CySEC-regulated entities (CIFs, UCITs, AIFMs) must comply with Cyprus's AML/CFT Law (L.188(I)/2007) as amended, transposing EU AMLD into national law. CySEC issues its own AML Directive with specific obligations for the investment sector.", rulePatterns: ["HRCOU", "DORMANT", "NCOU", "RISKWORD", "REFTEXT", "Large Transaction"], suggestedRules: [{ name: "[CYSEC] CIF suspicious transaction flag", severity: "high", rationale: "CySEC Directive DI144-2007-08 requires Cyprus Investment Firms to monitor and report suspicious transactions to MOKAS (FIU). This rule flags transactions with multiple risk indicators.", conditions: [{ field: "customer.riskScore", operator: ">", value: "6" }, { field: "transaction.amount", operator: ">", value: "5000" }] }] },
  { id: "cysec-cdd", regulation: "Customer Due Diligence (CySEC)", citation: "CySEC Directive DI144-2007-08 §4", description: "CIFs must apply CDD measures when establishing a business relationship, executing occasional transactions ≥ €15,000, or when there is suspicion of ML/TF. Risk-based approach with simplified measures for lower-risk and enhanced measures for higher-risk situations.", rulePatterns: ["VC", "CDC01-P", "CDC01-E"], suggestedRules: [{ name: "[CYSEC] EDD trigger for high-risk CIF clients", severity: "high", rationale: "CySEC's AML Directive §4 requires enhanced due diligence for high-risk clients of CIFs, including non-face-to-face relationships and clients from high-risk third countries.", conditions: [{ field: "customer.riskScore", operator: ">", value: "6" }] }] },
  { id: "cysec-mokas", regulation: "MOKAS Suspicious Transaction Reporting", citation: "L.188(I)/2007 Art 27 / MOKAS Circular", description: "All obliged entities must report suspicious transactions to MOKAS (Unit for Combating Money Laundering) without delay. There is no minimum threshold. Reports must include full customer identification, transaction details, and grounds for suspicion.", rulePatterns: ["HRCOU", "DORMANT", "IN>AVG", "OUT>AVG"], suggestedRules: [{ name: "[CYSEC-STR] MOKAS reporting — suspicious activity", severity: "critical", rationale: "L.188(I)/2007 Art 27 requires immediate reporting to MOKAS when there are reasonable grounds for suspicion. No minimum threshold applies. This rule flags high-risk-score transactions for STR filing.", conditions: [{ field: "customer.riskScore", operator: ">", value: "7" }] }, { name: "[CYSEC-STR] Unusual transaction pattern for MOKAS", severity: "high", rationale: "MOKAS guidance identifies unusual patterns (dormant account reactivation, round amounts, rapid succession) as key indicators requiring STR filing.", conditions: [{ field: "transaction.frequency", operator: ">", value: "5" }, { field: "transaction.amount", operator: ">", value: "3000" }] }] },
  { id: "cysec-pep", regulation: "PEP Obligations (Cyprus)", citation: "CySEC Directive DI144-2007-08 §5 / L.188(I)/2007 Art 22", description: "CIFs must determine whether customers or beneficial owners are PEPs (including domestic PEPs). For PEPs: senior management approval, source of wealth/funds verification, and enhanced ongoing monitoring. Extended to family members and known close associates.", rulePatterns: ["CUSTPEP", "CTPYPEP"], suggestedRules: [{ name: "[CYSEC] PEP enhanced monitoring (Cyprus domestic)", severity: "high", rationale: "Under Cyprus law, domestic PEPs require the same enhanced measures as foreign PEPs, including senior management approval and source of wealth verification.", conditions: [{ field: "customer.pepStatus", operator: "IN", value: "pep,domestic_pep" }] }] },
  { id: "cysec-sanctions", regulation: "EU/Cyprus Sanctions Compliance", citation: "EU Council Regulations / L.58(I)/2016", description: "CySEC-regulated entities must comply with all EU restrictive measures and UN Security Council sanctions as implemented in Cyprus. Screening against EU Consolidated Sanctions List and OFAC (where US nexus exists) is mandatory.", rulePatterns: ["CUSTSCRS", "CUSTSCRH", "CTPYSCRS", "CUSTBIC", "CTPYBIC", "INSTSCRS", "INSTSCRH", "High-Risk Country"], suggestedRules: [{ name: "[CYSEC] EU + Cyprus sanctions screening", severity: "critical", rationale: "L.58(I)/2016 requires compliance with EU restrictive measures. CIFs must screen all customers and counterparties against EU and UN sanctions lists.", conditions: [{ field: "customer.status", operator: "==", value: "sanctions_match" }] }] },
  { id: "cysec-ubo", regulation: "UBO Register & Verification (Cyprus)", citation: "L.188(I)/2007 Art 20 / Companies Law Cap 113", description: "Cyprus transposed the EU UBO register requirement. CIFs must identify and verify beneficial owners with a 25%+1 share threshold. Cross-referencing the Cyprus Registrar of Companies (DRCOR) UBO register is expected.", rulePatterns: ["MCOC", "OCMC"], suggestedRules: [{ name: "[CYSEC] UBO verification via DRCOR register", severity: "high", rationale: "Cyprus law requires CIFs to verify beneficial owners against the DRCOR UBO register. This rule flags entity customers with incomplete UBO records.", conditions: [{ field: "customer.status", operator: "==", value: "ubo_incomplete" }] }] },
  { id: "cysec-record", regulation: "Record Keeping (CySEC — 5 Years)", citation: "CySEC Directive DI144-2007-08 §8", description: "CIFs must retain CDD documentation and transaction records for at least 5 years after the business relationship has ended. Records must be sufficient to reconstruct individual transactions and must be available to MOKAS and CySEC on request.", rulePatterns: [] },
  { id: "cysec-training", regulation: "Staff Training & Compliance Officer", citation: "CySEC Directive DI144-2007-08 §9", description: "CIFs must appoint a designated AML Compliance Officer (AMLCO) at management level, ensure regular staff training on AML/CFT obligations, and conduct independent audits of their AML programme at least annually.", rulePatterns: [], suggestedRules: [{ name: "[CYSEC] Annual AML audit review trigger", severity: "medium", rationale: "CySEC requires CIFs to conduct annual independent audits of their AML compliance programme. This rule triggers the review cycle.", conditions: [{ field: "alert.count", operator: ">=", value: "0" }] }] },
  { id: "cysec-risk", regulation: "Risk-Based Approach (CySEC)", citation: "CySEC Directive DI144-2007-08 §3", description: "CIFs must adopt a risk-based approach proportionate to their size and nature. They must document their risk assessment covering customer types, geographic risk, product/service risk, and delivery channel risk. CySEC may request evidence of the assessment.", rulePatterns: ["HRCOU", "NCOU", "PGAV-IN", "PGAV-OUT"], suggestedRules: [{ name: "[CYSEC] Quarterly risk assessment review", severity: "medium", rationale: "CySEC expects CIFs to keep their risk assessments current. This rule triggers quarterly reviews to ensure documentation reflects evolving risk factors.", conditions: [{ field: "alert.count", operator: ">=", value: "0" }] }] },
  { id: "cysec-tpf", regulation: "Third-Party Funds & Payments", citation: "CySEC Circular C433 / DI144-2007-08", description: "CIFs must exercise heightened vigilance for third-party payments and ensure that funds are received from/sent to accounts in the client's own name. Payments from unrelated third parties require enhanced scrutiny and documentation of the relationship.", rulePatterns: [], suggestedRules: [{ name: "[CYSEC] Third-party payment detection", severity: "high", rationale: "CySEC Circular C433 requires enhanced scrutiny of third-party payments. This rule flags transactions where the payment source differs from the account holder.", conditions: [{ field: "transaction.type", operator: "==", value: "third_party" }] }] },
];

type RegKey = "fincen" | "fintrac" | "fca" | "eu_amld" | "cysec";

const REGULATOR_MAP: Record<string, { key: RegKey; label: string; color: string; activeColor: string }> = {
  ...
  eu_amld: { key: "eu_amld", label: "EU AMLD", color: "border-violet-300 bg-violet-50 text-violet-700 hover:bg-violet-100", activeColor: "border-violet-500 bg-violet-100 text-violet-800" },
  cysec: { key: "cysec", label: "CySEC", color: "border-teal-300 bg-teal-50 text-teal-700 hover:bg-teal-100", activeColor: "border-teal-500 bg-teal-100 text-teal-800" },
  icpac: { key: "eu_amld", label: "ICPAC", color: "border-violet-300 bg-violet-50 text-violet-700 hover:bg-violet-100", activeColor: "border-violet-500 bg-violet-100 text-violet-800" },
  cba_cyprus: { key: "cysec", label: "CBA Cyprus", color: "border-teal-300 bg-teal-50 text-teal-700 hover:bg-teal-100", activeColor: "border-teal-500 bg-teal-100 text-teal-800" },
};
...
export default function SuiteAlertRules() {
  ...
  return (
    <div className="flex h-full overflow-hidden bg-background">
      ...
      <div className="w-72 shrink-0 border-r border-border flex flex-col bg-card">
        <div className="px-4 py-3 border-b border-border flex flex-col gap-2 shrink-0">
          ...
          {showOtherRegs && (
            <div className="space-y-1">
              {(["fincen", "fintrac", "fca", "eu_amld", "cysec"] as RegKey[]).filter(k => !userRegulator || REGULATOR_MAP[userRegulator]?.key !== k).map(k => {
                const label = k === "fincen" ? "FinCEN / BSA" : k === "fintrac" ? "FINTRAC" : k === "fca" ? "FCA (UK)" : k === "eu_amld" ? "EU AMLD" : "CySEC";
                const rm = REGULATOR_MAP[k];
                const colors = { normal: rm.color, active: rm.activeColor };
                return (
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1.5">
          ...
          {rules.map(r => {
            ...
            const regLabel = r.name.includes("[FINCEN") ? "FinCEN" : r.name.includes("[FINTRAC") ? "FINTRAC" : r.name.includes("[FCA") ? "FCA" : r.name.includes("[EU-AMLD") || r.name.includes("[EU-WTR") ? "EU AMLD" : r.name.includes("[CYSEC") ? "CySEC" : r.name.includes("[EU") ? "EU" : null;
            const regBadgeColor = regLabel === "FinCEN" ? "bg-blue-100 text-blue-700" : regLabel === "FINTRAC" ? "bg-red-100 text-red-700" : regLabel === "FCA" ? "bg-indigo-100 text-indigo-700" : regLabel === "EU AMLD" ? "bg-violet-100 text-violet-700" : regLabel === "CySEC" ? "bg-teal-100 text-teal-700" : regLabel === "EU" ? "bg-violet-100 text-violet-700" : "";
            return (
      ...
      {/* Right panel - EU AMLD Mapping */}
      {activeRegPanel === "eu_amld" && (
        <div className="w-[460px] shrink-0 border-l border-border flex flex-col bg-card overflow-hidden">
          <div className="px-4 py-3 border-b border-border flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2"><Scale className="w-4 h-4 text-violet-600" /><h3 className="font-semibold text-foreground text-sm">EU AMLD Regulatory Mapping</h3></div>
            <button onClick={() => setActiveRegPanel(null)} className="p-1 rounded hover:bg-muted transition-colors"><X className="w-4 h-4 text-muted-foreground" /></button>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {(() => {
              const totalReqs = EU_AMLD_REQUIREMENTS.length;
              const coveredReqs = EU_AMLD_REQUIREMENTS.filter(req => req.rulePatterns.length > 0 && req.rulePatterns.some(p => rules.some(r => r.name.includes(p)))).length;
              const coveragePct = Math.round((coveredReqs / totalReqs) * 100);
              const allSuggested = EU_AMLD_REQUIREMENTS.flatMap(r => r.suggestedRules || []);
              return (
                <div className="bg-gradient-to-br from-violet-50 to-purple-50 border border-violet-200 rounded-xl p-4">
                  <div className="flex items-end gap-4 mb-3">
                    <div><p className="text-[10px] text-violet-600 uppercase tracking-wider font-semibold mb-1">EU AMLD Coverage</p><div className="flex items-baseline gap-1"><span className="text-3xl font-bold text-violet-900">{coveragePct}%</span></div></div>
                    <div className="flex-1 grid grid-cols-3 gap-2">
                      <div className="text-center"><p className="text-[10px] text-muted-foreground mb-0.5">Covered</p><p className="text-xs font-bold text-emerald-600">{coveredReqs}/{totalReqs}</p></div>
                      <div className="text-center"><p className="text-[10px] text-muted-foreground mb-0.5">Gaps</p><p className="text-xs font-bold text-red-600">{totalReqs - coveredReqs}</p></div>
                      <div className="text-center"><p className="text-[10px] text-muted-foreground mb-0.5">Suggested</p><p className="text-xs font-bold text-amber-600">{allSuggested.length}</p></div>
                  ...
                  <div className="w-full bg-violet-200 rounded-full h-2"><div className="bg-violet-600 h-2 rounded-full transition-all" style={{ width: `${coveragePct}%` }} /></div>
                </div>
              );
            })()}
            {EU_AMLD_REQUIREMENTS.map(req => {
              const matchedRules = rules.filter(r => req.rulePatterns.some(p => r.name.includes(p)));
              const isCovered = matchedRules.length > 0;
              const hasSuggestions = (req.suggestedRules?.length || 0) > 0;
              return (
                <div key={req.id} className={cn("border rounded-xl overflow-hidden", isCovered ? "border-emerald-200" : "border-red-200")}>
                  <div className={cn("px-4 py-3 flex items-start gap-3", isCovered ? "bg-emerald-50/50" : "bg-red-50/50")}>
                    {isCovered ? <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" /> : <AlertOctagon className="w-4 h-4 text-red-500 shrink-0 mt-0.5" />}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-1"><h4 className="text-xs font-semibold text-foreground">{req.regulation}</h4><span className="text-[10px] px-2 py-0.5 rounded-full bg-violet-100 text-violet-700 border border-violet-200 font-mono font-medium shrink-0">{req.citation}</span></div>
                      <p className="text-[11px] text-muted-foreground leading-relaxed">{req.description}</p>
                    </div>
                  </div>
                  {matchedRules.length > 0 && (
      ...
      {/* Right panel - CySEC Mapping */}
      {activeRegPanel === "cysec" && (
        <div className="w-[460px] shrink-0 border-l border-border flex flex-col bg-card overflow-hidden">
          <div className="px-4 py-3 border-b border-border flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2"><Scale className="w-4 h-4 text-teal-600" /><h3 className="font-semibold text-foreground text-sm">CySEC Regulatory Mapping</h3></div>
            <button onClick={() => setActiveRegPanel(null)} className="p-1 rounded hover:bg-muted transition-colors"><X className="w-4 h-4 text-muted-foreground" /></button>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {(() => {
              const totalReqs = CYSEC_REQUIREMENTS.length;
              const coveredReqs = CYSEC_REQUIREMENTS.filter(req => req.rulePatterns.length > 0 && req.rulePatterns.some(p => rules.some(r => r.name.includes(p)))).length;
              const coveragePct = Math.round((coveredReqs / totalReqs) * 100);
              const allSuggested = CYSEC_REQUIREMENTS.flatMap(r => r.suggestedRules || []);
              return (
                <div className="bg-gradient-to-br from-teal-50 to-cyan-50 border border-teal-200 rounded-xl p-4">
                  <div className="flex items-end gap-4 mb-3">
                    <div><p className="text-[10px] text-teal-600 uppercase tracking-wider font-semibold mb-1">CySEC Coverage</p><div className="flex items-baseline gap-1"><span className="text-3xl font-bold text-teal-900">{coveragePct}%</span></div></div>
                    <div className="flex-1 grid grid-cols-3 gap-2">
                      <div className="text-center"><p className="text-[10px] text-muted-foreground mb-0.5">Covered</p><p className="text-xs font-bold text-emerald-600">{coveredReqs}/{totalReqs}</p></div>
                      <div className="text-center"><p className="text-[10px] text-muted-foreground mb-0.5">Gaps</p><p className="text-xs font-bold text-red-600">{totalReqs - coveredReqs}</p></div>
                      <div className="text-center"><p className="text-[10px] text-muted-foreground mb-0.5">Suggested</p><p className="text-xs font-bold text-amber-600">{allSuggested.length}</p></div>
                  ...
                  <div className="w-full bg-teal-200 rounded-full h-2"><div className="bg-teal-600 h-2 rounded-full transition-all" style={{ width: `${coveragePct}%` }} /></div>
                </div>
              );
            })()}
            {CYSEC_REQUIREMENTS.map(req => {
              const matchedRules = rules.filter(r => req.rulePatterns.some(p => r.name.includes(p)));
              const isCovered = matchedRules.length > 0;
              const hasSuggestions = (req.suggestedRules?.length || 0) > 0;
              return (
                <div key={req.id} className={cn("border rounded-xl overflow-hidden", isCovered ? "border-emerald-200" : "border-red-200")}>
                  <div className={cn("px-4 py-3 flex items-start gap-3", isCovered ? "bg-emerald-50/50" : "bg-red-50/50")}>
                    {isCovered ? <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" /> : <AlertOctagon className="w-4 h-4 text-red-500 shrink-0 mt-0.5" />}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-1"><h4 className="text-xs font-semibold text-foreground">{req.regulation}</h4><span className="text-[10px] px-2 py-0.5 rounded-full bg-teal-100 text-teal-700 border border-teal-200 font-mono font-medium shrink-0">{req.citation}</span></div>
                      <p className="text-[11px] text-muted-foreground leading-relaxed">{req.description}</p>
                    </div>
                  </div>
                  {matchedRules.length > 0 && (
      ...
      {showBacktest && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setShowBacktest(false)}>
          <div className="bg-card rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <div className="flex items-center gap-3">
                <TrendingUp className="w-5 h-5 text-emerald-600" />
                <div>
                  <h3 className="font-semibold text-foreground text-sm">Rule Backtest — {backtestResult?.rule?.name || activeRule?.name}</h3>
                  <p className="text-xs text-muted-foreground">Preview alerts this rule would produce against historical transactions before enabling.</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <select value={backtestDays} onChange={e => { const d = Number(e.target.value); setBacktestDays(d); runBacktest(d); }} className="text-xs border border-border rounded-lg px-2 py-1.5 bg-card">
                  <option value={7}>Last 7 days</option>
                  <option value={30}>Last 30 days</option>
                  <option value={90}>Last 90 days</option>
                  <option value={180}>Last 180 days</option>
                </select>
                <button onClick={() => setShowBacktest(false)} className="p-1.5 rounded-lg hover:bg-muted"><X className="w-4 h-4" /></button>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {backtesting || !backtestResult ? (
                <div className="flex items-center justify-center py-16 text-muted-foreground text-sm gap-2">
                  <Loader2 className="w-4 h-4 animate-spin" />Running backtest against {backtestDays} days of transactions…
                </div>
              ) : (
                <>
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                    <div className="rounded-xl border border-border bg-muted/30 p-3">
                      <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Evaluated</div>
                      <div className="text-xl font-bold text-foreground mt-1">{backtestResult.evaluated.toLocaleString()}</div>
                      <div className="text-[10px] text-muted-foreground mt-0.5">transactions</div>
                    </div>
                    <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3">
                      <div className="text-[10px] uppercase tracking-wider text-emerald-700 font-semibold">Would Alert</div>
                      <div className="text-xl font-bold text-emerald-800 mt-1">{backtestResult.total_matches.toLocaleString()}</div>
                      <div className="text-[10px] text-emerald-700 mt-0.5">{backtestResult.match_rate_pct}% match rate</div>
                    </div>
                    <div className="rounded-xl border border-border bg-muted/30 p-3">
                      <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Daily Avg</div>
                      <div className="text-xl font-bold text-foreground mt-1">{backtestResult.daily_average}</div>
                      <div className="text-[10px] text-muted-foreground mt-0.5">alerts/day</div>
                    </div>
                    <div className="rounded-xl border border-border bg-muted/30 p-3">
                      <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Unique Customers</div>
                      <div className="text-xl font-bold text-foreground mt-1">{backtestResult.unique_customers}</div>
                      <div className="text-[10px] text-muted-foreground mt-0.5">would be flagged</div>
                    </div>
                    <div className={cn("rounded-xl border p-3", backtestResult.fp_estimate_pct > 50 ? "bg-red-50 border-red-200" : backtestResult.fp_estimate_pct > 25 ? "bg-amber-50 border-amber-200" : "bg-emerald-50 border-emerald-200")}>
                      <div className="text-[10px] uppercase tracking-wider font-semibold text-muted-foreground">Est. False Positives</div>
                      <div className="text-xl font-bold mt-1">{backtestResult.fp_estimate_pct}%</div>
                      <div className="text-[10px] text-muted-foreground mt-0.5">low/med-risk hits</div>
                    </div>
                  </div>

                  {backtestResult.total_matches > 0 && backtestResult.match_rate_pct > 20 && (
                    <div className="flex items-start gap-2 p-3 rounded-xl border border-amber-200 bg-amber-50">
                      ...
                      <p className="text-xs text-amber-800">High match rate ({backtestResult.match_rate_pct}%). This rule may generate significant alert volume — consider tightening thresholds before enabling.</p>
                    </div>
                  )}
                  {backtestResult.total_matches === 0 && (
                    <div className="flex items-start gap-2 p-3 rounded-xl border border-blue-200 bg-blue-50">
                      <AlertCircle className="w-4 h-4 text-blue-600 shrink-0 mt-0.5" />
                      <p className="text-xs text-blue-800">No matches in the last {backtestResult.window_days} days. Rule is either well-scoped or conditions are too strict — verify with sample data.</p>
                    </div>
                  )}

                  <div className="grid md:grid-cols-2 gap-4">
                    <div className="rounded-xl border border-border overflow-hidden">
                      <div className="px-4 py-2 border-b border-border bg-muted/30"><h4 className="text-xs font-semibold text-foreground">Top flagged customers</h4></div>
                      <div className="divide-y divide-border">
                        {backtestResult.top_customers.length === 0 && <div className="p-4 text-xs text-muted-foreground text-center">No customer hits</div>}
                        {backtestResult.top_customers.map((c: any) => (
                          <div key={c.customer_id} className="flex items-center justify-between px-4 py-2">
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="text-[10px] text-muted-foreground capitalize">Risk: {c.risk_level}</div>
                            </div>
                            <span className="text-xs font-bold text-foreground shrink-0">{c.hits}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div className="rounded-xl border border-border overflow-hidden">
                      <div className="px-4 py-2 border-b border-border bg-muted/30"><h4 className="text-xs font-semibold text-foreground">Top counterparty countries</h4></div>
                      <div className="divide-y divide-border">
                        {backtestResult.top_countries.length === 0 && <div className="p-4 text-xs text-muted-foreground text-center">No geographic data</div>}
                        {backtestResult.top_countries.map((c: any) => (
                          <div key={c.country} className="flex items-center justify-between px-4 py-2">
                            <div className="flex items-center gap-2"><Globe className="w-3 h-3 text-muted-foreground" /><span className="text-xs font-medium text-foreground font-mono">{c.country}</span></div>
                            <span className="text-xs font-bold text-foreground">{c.hits}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="rounded-xl border border-border overflow-hidden">
                    <div className="px-4 py-2 border-b border-border bg-muted/30 flex items-center justify-between">
                      <h4 className="text-xs font-semibold text-foreground">Sample matched transactions</h4>
                      <span className="text-[10px] text-muted-foreground">Showing {backtestResult.sample_matches.length} of {backtestResult.total_matches}</span>
                    </div>
                    <div className="max-h-72 overflow-y-auto">
                      <table className="w-full text-xs">
                        <thead className="bg-muted/20 sticky top-0"><tr className="text-left text-[10px] uppercase tracking-wider text-muted-foreground"><th className="px-3 py-2">Date</th><th className="px-3 py-2">Customer</th><th className="px-3 py-2">Amount</th><th className="px-3 py-2">Dir</th><th className="px-3 py-2">Counterparty</th><th className="px-3 py-2">Country</th></tr></thead>
                        <tbody className="divide-y divide-border">
                          {backtestResult.sample_matches.map((m: any) => (
                            <tr key={m.tx_id} className="hover:bg-muted/30">
                              <td className="px-3 py-2 text-muted-foreground whitespace-nowrap">{new Date(m.created_at).toLocaleDateString()}</td>
                              <td className="px-3 py-2 text-foreground truncate max-w-[140px]">{m.customer_name || m.customer_id.slice(0,8)}</td>
                              <td className="px-3 py-2 font-mono text-foreground">{Number(m.amount).toLocaleString()} {m.currency}</td>
                              <td className="px-3 py-2 capitalize text-muted-foreground">{m.direction}</td>
                              <td className="px-3 py-2 text-muted-foreground truncate max-w-[160px]">{m.counterparty || "—"}</td>
                              <td className="px-3 py-2 font-mono text-muted-foreground">{m.counterparty_country || "—"}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  {activeRule && !activeRule.enabled && backtestResult.total_matches > 0 && (
                    <div className="flex items-center justify-between p-3 rounded-xl border border-primary/20 bg-primary/5">
                      <div>
                        <p className="text-xs font-semibold text-foreground">Ready to enable?</p>
                        <p className="text-[10px] text-muted-foreground">These are preview-only — no alerts have been created.</p>
                      </div>
                      <button
                        onClick={async () => {
                          await supabase.from("suite_alert_rules").update({ is_active: true }).eq("id", activeRule.id);

...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Partner Program invite is NOT sent immediately on registration.
    // It is dispatched 1 day after signup by the send-signup-followup cron job.
    // Immediate partner invites are only sent when a user explicitly requests
    // partnership via the Contact Sales form (see src/pages/ContactSales.tsx).
